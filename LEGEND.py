...
async def plan(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="*❌ You are not authorized to view the price list!*",
            parse_mode='Markdown'
        )
        return

    # Price list message with emojis
    price_list = (
        "*💥 Checkout Our Botnet Rates 💥*
...
"
    ...
    # Send the price list to the admin
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=price_list,
        parse_mode='Markdown'
...
"
        "1️⃣ **Don't Run Too Many Attacks:** Doing so can result in a ban from the bot.
"
        "2️⃣ **Don't Run 2 Attacks At the Same Time:** If you do, you may get banned from the bot.
"
        "3️⃣ **Logs are Monitored Daily:** Follow these rules to avoid getting banned.

"
        "*✅ Follow the rules to use the bot safely!*"
    )

    # Send the rules message to the user
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=rules_message,
        parse_mode='Markdown'
...
# Dictionary to track user spam counts
user_spam_counts = {}

# Spam detection and warning function
async def detect_spam(update: Update, context: CallbackContext):
    ...
    if not user:
        ...
        if user_id not in user_spam_counts:
            user_spam_counts[user_id] = 1
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="*BHAI BAR BAR SPAM MAT KAR BOT KO USE KARNA HAI DM KAR 30 MINUTES FREE TRIAL MIL JAYEGA*",
                parse_mode='Markdown'
            )
        else:
            user_spam_counts[user_id] += 1

        # Send second warning if spam continues
        if user_spam_counts[user_id] == 2:
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="*ABE MADARCHOD CHOD Q SPAM KAR RAHA HAI*",
                parse_mode='Markdown'
            )
def main():
